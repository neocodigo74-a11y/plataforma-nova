...
interface Formulario {
  ...
  empresas?: { nome: string } | null; // <-- objeto único ou null
}
...
export default function descubrir() {
  ...
  const fetchFormularios = async () => {
    ...
    if (!error && data) {
      // Mapear para pegar apenas a primeira empresa
      const formulariosFormatados = data.map((f: any) => ({
        id: f.id,
        titulo: f.titulo,
        ...
        premiado: f.premiado,
        limitado: f.limitado,
        total_candidatos: f.total_candidatos,
        empresas: f.empresas?.[0] || null, // <-- pega a primeira empresa ou null
      }));

      setFormularios(formulariosFormatados);
      fetchParticipantes(formulariosFormatados);
    }
  ...
  onClick={() => router.push(`/Detalhes/DetalhesFormulario/${item.id}`)}
>
  Ver detalhes →
</button>
