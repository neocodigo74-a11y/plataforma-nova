...
export default function ChatPage({ userId, contatoId, onBack }: ChatPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const { data } = await supabase
        ...
        .select("*")
        .or(
          `and(remetente_id.eq.${user.id},destinatario_id.eq.${contatoId}),and(remetente_id.eq.${contatoId},destinatario_id.eq.${user.id})`
        )
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="sticky top-0 z-20 flex items-center px-4 py-2 border-b bg-white">
        <LucideArrowLeft size={24} className="cursor-pointer" onClick={onBack} />
        ...
        <LucideLock size={18} className="text-gray-500" />
      </div>
          ...
          <div key={date} className="mb-6">
            ...
            <AnimatePresence>
              {grouped[date].map((msg: Message) => {
                const isSent = msg.remetente_id === user?.id;
      ...
      {/* ================= INPUT FIXO ================= */}
      <div className="sticky bottom-0 z-20 flex items-center px-4 py-19  bg-white">
